...
/** POST /tienda/auth/verificar-token */
async function verificarTokenAcceso(req, res) {
    const { token } = req.body;
    if (!token) return Respuesta.error(res, 'Token requerido', 400);
    try {
        const decoded = jwt.verify(token, process.env.JWT_SECRET);
        const acceso = await DashboardService.verificarAccesoTienda(decoded.id_usuario);
        if (!acceso) return Respuesta.error(res, 'No tienes acceso al módulo de tienda.', 403);
        ...
        console.error('[Tienda] verificarToken:', err.message);
        return Respuesta.error(res, 'Error al verificar el token.');
    }
}

/** POST /tienda/auth/generar-codigo */
async function generarCodigoAcceso(req, res) {
    ...
    try {
        ...
        if (err.name === 'TokenExpiredError') return Respuesta.error(res, 'El token ha expirado.', 401);
        if (err.name === 'JsonWebTokenError')  return Respuesta.error(res, 'Token inválido.', 401);
        console.error('[Tienda] generarCodigoAcceso:', err.message);
        return Respuesta.error(res, 'Error al generar código de acceso.');
    }
}

/** POST /tienda/auth/canjear-codigo */
async function canjearCodigo(req, res) {
    ...
    try {
        const acceso = await DashboardService.verificarAccesoTienda(entry.idUsuario);
        ...
        console.error('[Tienda] canjearCodigo:', err.message);
        return Respuesta.error(res, 'Error al canjear código.');
    }
}

/** GET /tienda/dashboard/resumen?id_negocio=N */
async function getResumen(req, res) {
    try {
        ...
        console.error('[Tienda] getResumen:', err.message);
...
async function getPerfil(req, res) {
    try {
        const acceso = await DashboardService.verificarAccesoTienda(req.usuario.id_usuario);
        if (!acceso) return Respuesta.error(res, 'No tienes acceso al módulo de tienda.', 403);
        return Respuesta.success(res, 'Perfil obtenido', acceso);
    } catch (err) {
        console.error('[Tienda] getPerfil:', err.message);
