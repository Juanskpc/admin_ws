...
async function verificarTokenAcceso(req, res) {
    ...
    try {
        ...
        const acceso = await DashboardService.verificarAccesoReserva(decoded.id_usuario);
        if (!acceso) return Respuesta.error(res, 'No tienes acceso al módulo de reservas.', 403);
        ...
        console.error('[Reserva] verificarToken:', err.message);
        return Respuesta.error(res, 'Error al verificar el token.');
    }
}

/** POST /reserva/auth/generar-codigo */
async function generarCodigoAcceso(req, res) {
    ...
    try {
        ...
        if (err.name === 'TokenExpiredError') return Respuesta.error(res, 'El token ha expirado.', 401);
        if (err.name === 'JsonWebTokenError')  return Respuesta.error(res, 'Token inválido.', 401);
        console.error('[Reserva] generarCodigoAcceso:', err.message);
        return Respuesta.error(res, 'Error al generar código de acceso.');
    }
}

/** POST /reserva/auth/canjear-codigo */
async function canjearCodigo(req, res) {
    ...
    try {
        const acceso = await DashboardService.verificarAccesoReserva(entry.idUsuario);
        ...
        console.error('[Reserva] canjearCodigo:', err.message);
        return Respuesta.error(res, 'Error al canjear código.');
    }
}

/** GET /reserva/dashboard/resumen?id_negocio=N */
async function getResumen(req, res) {
    try {
        ...
        console.error('[Reserva] getResumen:', err.message);
...
async function getPerfil(req, res) {
    try {
        const acceso = await DashboardService.verificarAccesoReserva(req.usuario.id_usuario);
        if (!acceso) return Respuesta.error(res, 'No tienes acceso al módulo de reservas.', 403);
        return Respuesta.success(res, 'Perfil obtenido', acceso);
    } catch (err) {
        console.error('[Reserva] getPerfil:', err.message);
